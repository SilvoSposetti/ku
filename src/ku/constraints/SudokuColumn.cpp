...
std::unique_ptr<Group> SudokuColumn::getSvgGroup(const DrawingOptions& options) const {
  ...
  for (int32_t i = 0; i < Sudo::MAX_DIGIT; i++) {
    ...
    group->add(std::make_unique<Line>(x, 0.0, x, options.size, std::nullopt, std::nullopt));
  }
  return group;
}
